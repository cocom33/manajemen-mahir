        ...
        <div class="p-5 intro-y box">
          <form method="POST" action="{{ route('notes.update', $note->id) }}">
            ...
            @method('PUT')  
                <div>
                    <label>Title</label>
                    <input type="text" name="title" value="{{ $note->title }}" class="input w-full border mt-2 @error('title') border-theme-6 @enderror" placeholder="Input Title">
                    @error('title')
                ...
                <div class="mt-5 form-group">
                    <label>Content</label>
                    <textarea class="relative z-50" id="summernote" name="description">{{ $note->description }}</textarea>
                </div>
